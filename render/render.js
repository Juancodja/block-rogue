...
enemySprite.src = 'resources/enemy.png'

const playerSprite = new Image();
playerSprite.src = 'resources/player.png'
...
function drawEntities() {
    ...
    enemies.forEach(e => {
        ctx.drawImage(enemySprite, e.x - e.width / 2, e.y - e.height / 2, e.width, e.height);
    });

    players.forEach(p => {
        ctx.fillStyle = p.color;
        ctx.drawImage(playerSprite, p.x - p.width / 2, p.y - p.height / 2, p.width, p.height);
    });
